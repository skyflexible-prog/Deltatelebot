...
class ShortStrangleBot:
    ...
    async def telegram_positions(self, update, context):
    ...
    async def execute_short_strangle(self) -> Dict:
        ...
        try:
            ...
            await self.send_telegram_message(f"❌ <b>Trade Execution Failed</b>\n\nError: {str(e)}")
            return {"success": False, "error": str(e)}
    ...
    def start_scheduler(self):
        ...
        scheduler.add_job(
            func=lambda: asyncio.create_task(self.execute_short_strangle()),
        ...
        scheduler.add_job(
            func=lambda: asyncio.create_task(self.monitor_positions()),
            trigger='interval',
            minutes=5,
            id='position_monitor',
...
def telegram_webhook():
    ...
    try:
        if bot_instance and bot_instance.telegram_app:
            update = telegram.Update.de_json(request.get_json(), bot_instance.telegram_app.bot)
            asyncio.create_task(bot_instance.telegram_app.process_update(update))
...
def manual_execute():
    ...
    try:
        if bot_instance:
            result = asyncio.create_task(bot_instance.execute_short_strangle())
...
def main():
    ...
    # Initialize bot
    bot_instance = ShortStrangleBot(API_KEY, API_SECRET, TELEGRAM_TOKEN, CHAT_ID)
    
    # Start scheduler
    scheduler = bot_instance.start_scheduler()
    ...
    asyncio.create_task(bot_instance.send_telegram_message(
